...
class PrefUtil {
    companion object {
        ...
        fun getTimerState(context: Context): AccessActivity.TimerState{
            val preferences = PreferenceManager.getDefaultSharedPreferences(context)
            val ordinal = preferences.getInt(TIMER_STATE_ID, 0)
            return AccessActivity.TimerState.values()[ordinal]
        }

        fun setTimerState(state: AccessActivity.TimerState, context: Context){
            val editor = PreferenceManager.getDefaultSharedPreferences(context).edit()
